...
Usage:
    python3 merge_native_image_config.py

The script reads:
    target/native-image-config/reachability-metadata.json  (source)
...
SEATA_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, *([".."] * 8)))

TARGET_DIR = os.path.join(SCRIPT_DIR)  # same directory as the config files
...
def main() -> int:
    ...
    for key, (filename, (src_key, tgt_key), fmt) in KEY_TO_FILE.items():
        ...
        if not os.path.isfile(target_path):
            print(f"[SKIP] '{key}' — target file missing: {target_path}")
            continue
